        ...
        {globalState.profileObj &&
            <div className="sm:hidden fixed bottom-0 left-0 z-50 w-full h-16 bg-white border-t border-gray-200 dark:bg-gray-700 dark:border-gray-600">
                <div className="grid h-full max-w-lg grid-cols-3 mx-auto font-medium">
                    {/* Home */}
                    <Link href="/dashboard" onClick={() => setGlobalState({ ...globalState, "surfing": !globalState.surfing })}>
                        {active && (() => {
                            let blue = checkActive(1);

                            return <button type="button" className="inline-flex flex-col items-center justify-center px-5 hover:bg-gray-50 dark:hover:bg-gray-800 group">
                    ...
                    <Link href="/dashboard/watchlist" onClick={() => setGlobalState({ ...globalState, "surfing": !globalState.surfing })}>
                        {active && (() => {
                    ...
                    {/* Profile */}
                    <Link href="/dashboard/profile" onClick={() => setGlobalState({ ...globalState, "surfing": !globalState.surfing })}>
                        {active && (() => {
